...
import { humanFileSize, secondsToStringHuman } from "../../scripts/nyan/util";
/* Styles */
    ...
    return (
        <div class={style["video-controls-overlay"]}>
            ...
            {props.encode ? <div class={style["video-controls-overlay-text"]}>
                Video: <span class={style["video-controls-overlay-text-highlight"]}>{props.playerData.preset === "X264" ? "mp4" : "webm"}@{props.playerData.preset.toLowerCase()} ({Math.round(props.encode.videoBitrate / 1000)}kbits/s)</span>
            </div> : null}
            {props.encode ? <div class={style["video-controls-overlay-text"]}>
            ...
            {level ? <div class={style["video-controls-overlay-text"]}>
                HLS: <span class={style["video-controls-overlay-text-highlight"]}>{level.codecs} ({Math.round(level.bitrate/1000*8)}kbits/s)</span>
            </div> : null}
            {level ? <div class={style["video-controls-overlay-text"]}>
            ...
            <div class={style["video-controls-overlay-space"]} />
            {props.encode ? <div class={style["video-controls-overlay-text"]}>
                Viewport: <span class={style["video-controls-overlay-text-highlight"]}>{props.video?.offsetWidth}x{props.video?.offsetHeight} ({quality?.droppedVideoFrames} dropped out of {quality?.totalVideoFrames})</span>
            </div> : null}
            {props.encode ? <div class={style["video-controls-overlay-text"]}>
